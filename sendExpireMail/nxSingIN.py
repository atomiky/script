...
if __name__ == '__main__':
    f = open("/var/sendMail/script/data/nxUnSingIN.txt", "r")
    # f = open("../data/unSignUser.txt", "r")
    lines = f.readlines()
    for i in range(0, len(lines) - 1, 5):
        send_email(lines[i], 'nxkys001@outlook.com', 'nx_Admin')
        time.sleep(random.uniform(432, 450))
        send_email(lines[i + 1], 'mkndcyg@outlook.com', 'Bncas123456')
        time.sleep(random.uniform(420, 445))
        send_email(lines[i + 2], 'nmkyter@outlook.com', 'nx_Admin')
        time.sleep(random.uniform(410, 460))
        send_email(lines[i + 3], 'mnkyter@outlook.com', 'nx_Admin')
        time.sleep(random.uniform(400, 455))
        send_email(lines[i + 4], 'mktreyn@outlook.com', 'nx_Admin')
